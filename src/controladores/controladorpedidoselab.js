...
exports.Guardar = async (req, res) => {
  ...
  if (msj.errores.length > 0) {
    ...
    var buscarPedido = await modeloDetallePedidos.findOne({
      where: {
        idregistro: iddetallepedido,
      },
    });
    
    var buscarLlave = await modeloPedidosElaborados.findOne({
      where: {
        iddetallepedido: iddetallepedido
      }
    });
    if (buscarPedido && !buscarLlave) {
      try {
...
exports.GuardarBulk = async (req, res) => {
  ...
  if (msj.errores.length > 0) {
    ...
    try {
      await modeloPedidosElaborados.bulkCreate(PedidoElaborado);
      msj.estado = "correcto";
      msj.mensaje = "Se ha guardado el registro correctamente";
      msj.errores = "";
      MSJ(res, 200, msj);
  ...
  if (msj.errores.length > 0) {
    ...
    try {
      var buscardetalle = await modeloPedidosElaborados.findOne({
        //con el findone le indicamos que busque solo un dato
        where: {
          iddetallepedido: id, //le indicamos que busque el dato por medio del que indicamos en el query del thunder
        },
      });
      if (!buscardetalle) {
        //el signo de admiracion significa si esta vacio o es false
        msj.estado = "error";
